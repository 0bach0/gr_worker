...
function findType(id,type){
    return new Promise(function(resolve,reject){
        if(type==undefined){
            tokenService.getToken().then((token)=>{
                var url = baseUrl + id + '?metadata=1' + '&access_token=' + token;             
                fbService.requestFb(url).then((data)=>{
                    data = JSON.parse(data);
                    var postType = data.metadata.type;
                    resolve(postType);
                },
                (err)=>{reject(err);}
                );
